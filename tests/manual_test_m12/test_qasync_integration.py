"""
M12 手动测试：qasync 集成验证（技术决策 D003 最终方案）

目标：
1. 验证 @asyncSlot 装饰器的功能
2. 测试高频 Signal 发射（100 tokens/s）稳定性
3. 验证异常处理和资源清理
4. 集成 OpenAI SDK 流式调用

技术方案：
- 使用 qasync 库（Qt + asyncio 事件循环融合）
- @asyncSlot() 装饰器自动处理 async/await
- Signal 自动在主线程发射
...
try:
    ...
    print("[ERROR] PySide6 未安装，请运行: pip install PySide6")
    sys.exit(1)

try:
    from qasync import asyncSlot, QEventLoop
except ImportError:
    print("[ERROR] qasync 未安装，请运行: pip install qasync>=0.28.0")
    sys.exit(1)


class ChatViewModel(QObject):
    """使用 qasync @asyncSlot 的 ViewModel（M12 真实架构）"""
...
class TestWindow(QWidget):
    ...
    def init_ui(self):
        """初始化 UI"""
        self.setWindowTitle("M12 qasync 测试（技术决策 D003 最终方案）")
        self.setGeometry(100, 100, 700, 600)

        layout = QVBoxLayout()

        # 标题
        title_label = QLabel("[TEST] qasync @asyncSlot 稳定性测试")
        ...
        btn2 = QPushButton("测试 2: 高频 Signal（100 tokens/s，3s）[KEY TEST]")
    ...
    def prepare_test(self, test_name: str):
        ...
        self.token_count = 0
        self.text_edit.append(f"[TEST] 开始测试: {test_name}\n")
    ...
    def on_error(self, error_msg: str):
        ...
        self.stats_label.setText(f"[WARN] 错误: {error_msg}")


def main():
    """主函数"""
    print("[TEST] M12 qasync 集成测试（技术决策 D003 最终方案）\n")
    print("=" * 60)
    print("技术方案: qasync 库（@asyncSlot 装饰器）")
    print("优势: Qt + asyncio 事件循环融合，代码优雅")
    print("=" * 60 + "\n")
