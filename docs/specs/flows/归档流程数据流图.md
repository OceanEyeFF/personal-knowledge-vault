...
> 本图是数据流示意；运行顺序以真实、版本化的 `config/workflows/archive-url.yaml` 为准。操作启动时 Application / Kernel 先捕获一个严格校验的、不可变 `Config` snapshot，并从其 `RuntimeLayout` 取得 Vault、SQLite、向量与日志路径；步骤执行中不得重新读取全局配置。adapter 必须保留 `success/degraded/error`、`warnings` 与机器可读 `issues`。
...
graph TD
    ...
    AA --> AD[file_path<br/>&lt;data-root&gt;/vault/wechat/标题.md]
...
这里的 `<data-root>` 由 `PKV_DATA_ROOT` → 用户配置
`storage.data_root` → `%USERPROFILE%\\.pkv\\data` 选择。唯一可编辑业务
配置是 `%USERPROFILE%\\.pkv\\config.yaml`；`<data-root>/config/local.yaml`
仅记录 PKV 管理的无密钥 runtime snapshot（例如数据库/Embedding 合同），不参与归档
配置合并。
...
输入: { entry: Entry }
输出: {
  file_path: string,       // "<data-root>/vault/wechat/标题.md"
...
```json
{
  ...
  "data": {
    ...
    "tags": ["AI", "技术", "工作流"],
    "notes": "我的个人思考：...",
    "file_path": "<data-root>/vault/wechat/文章标题.md",
    "knowledge_id": 123,
    "vector_ids": [1, 2, 3]
  },
  "errors": [],
  ...
  "logs": [
    ...
    "[idea_sharpen] 用户输入笔记",
    "[store_entry] Markdown 保存成功: <data-root>/vault/wechat/文章标题.md",
