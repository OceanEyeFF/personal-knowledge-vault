...
function Invoke-W4ReleaseAuditScenario {
    ...
    $inventoryEvidence = Assert-W4ReleaseInventory `
        ...
        -ExpectedExecutablePaths @('app/pkv.exe', 'app/pkv-mcp.exe')
...
# GUI scenarios migrated to the separately versioned pkv-GUI repository.
# Headless CLI/MCP scenarios continue below.
...
function Invoke-W4OfflineTextArchiveScenario {
    ...
    $seed = Invoke-W4McpSeedText -RunContext $RunContext `
        -ScenarioContext $ScenarioContext -Text $note `
        -Title 'W4 合成离线知识条目' -EvidenceName 'mcp-archive-text'
    $oracle = [ordered]@{
        status = 'passed'
        workflow_terminal = [string]$seed.terminal
        storage_status = [string]$seed.storage_status
        knowledge_id = [int64]$seed.knowledge_id
        data_tree_sha256 = Get-W4DataStateDigest -ScenarioContext $ScenarioContext
    }
    Write-W4JsonFile -Path (Join-Path $ScenarioContext.Evidence 'oracle.json') -Value $oracle
    return $oracle
}
...
function Invoke-W4Bm25SearchScenario {
    ...
    $seed = Invoke-W4McpSeedText -RunContext $RunContext `
        -ScenarioContext $ScenarioContext -Text $note `
        -Title 'W4 BM25 合成条目' -EvidenceName 'mcp-seed'
    $knownTerm = 'artifact-e2e-orchid'
    $hit = Invoke-W4Process -FileName $ScenarioContext.PkvExe `
        -Arguments @('search', $knownTerm, '--strategy', 'bm25', '--format', 'json') `
        -WorkingDirectory $ScenarioContext.WorkingDirectory `
        -Environment $ScenarioContext.Environment `
        -EvidenceDirectory (Join-Path $ScenarioContext.Evidence 'cli-bm25-hit') `
        -ExpectedExitCodes @(0) -TimeoutSeconds 60
    $hitPayload = ConvertFrom-W4StrictJsonText -Text $hit.StandardOutput `
        -Label 'CLI BM25 known-hit response'
    Assert-W4JsonObjectFields -Object $hitPayload -RequiredFields @(
        'query', 'status', 'strategy', 'total', 'issues', 'results'
    ) -Label 'CLI BM25 known-hit response'
    if ([string]$hitPayload.query -cne $knownTerm -or
        [string]$hitPayload.status -cne 'success' -or
        [string]$hitPayload.strategy -cne 'bm25' -or
        [int]$hitPayload.total -lt 1) {
        throw 'CLI BM25 known-hit response did not prove the seeded index was searched'
    }
    $seedId = [int64]$seed.knowledge_id
    $matchedSeed = @($hitPayload.results | Where-Object {
        $_.PSObject.Properties.Name -contains 'knowledge_id' -and
        [int64]$_.knowledge_id -eq $seedId
    })
    if ($matchedSeed.Count -lt 1) {
        throw 'CLI BM25 known-hit response did not contain the seeded knowledge identity'
    }
    $noHitTerm = 'pkvnohitqzvxfj314159265358979'
    $noHit = Invoke-W4Process -FileName $ScenarioContext.PkvExe `
        -Arguments @('search', $noHitTerm, '--strategy', 'bm25', '--format', 'json') `
        -WorkingDirectory $ScenarioContext.WorkingDirectory `
        -Environment $ScenarioContext.Environment `
        -EvidenceDirectory (Join-Path $ScenarioContext.Evidence 'cli-bm25-no-hit') `
        -ExpectedExitCodes @(0) -TimeoutSeconds 60
    $noHitPayload = ConvertFrom-W4StrictJsonText -Text $noHit.StandardOutput `
        -Label 'CLI BM25 no-hit response'
    if ([string]$noHitPayload.status -cne 'no_hits' -or
        [string]$noHitPayload.strategy -cne 'bm25' -or
        [int]$noHitPayload.total -ne 0) {
        throw 'CLI BM25 no-hit response did not preserve the five-state contract'
    }
    $oracle = [ordered]@{
        status = 'passed'
        known_term = $knownTerm
        known_hit_total = [int]$hitPayload.total
        known_hit_knowledge_id = $seedId
        no_hit_status = [string]$noHitPayload.status
    }
    Write-W4JsonFile -Path (Join-Path $ScenarioContext.Evidence 'oracle.json') -Value $oracle
    return $oracle
}
